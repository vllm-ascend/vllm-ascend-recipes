"""launch_online_dp.py — external DP launcher, vendored from upstream.

Copied verbatim from vllm-project/vllm-ascend @ v0.23.0rc1,
examples/external_online_dp/launch_online_dp.py.

The multi-node PD recipes reference this script but do not embed it (unlike
DeepSeek-V4-Flash, which inlines a copy). controller.py injects this vendored
copy into plan.json when the recipe has no embedded version, so every pod gets
it without depending on the image layout.
"""
